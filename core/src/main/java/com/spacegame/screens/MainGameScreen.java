...
import com.spacegame.entities.Asteroid;
import com.spacegame.entities.Bullet;
import com.spacegame.entities.Explosion;
...
public class MainGameScreen implements Screen {
    ...
    public static final float MIN_ASTEROID_SPAWN_TIME = 0.35f;
    public static final float MAX_ASTEROID_SPAWN_TIME = 0.85f;

    private float currentMinSpawnTime = 0.35f; // Tempo inicial mínimo (pode ajustar)
    private float currentMaxSpawnTime = 0.85f; // Tempo inicial máximo (pode ajustar)
    private final float DIFFICULTY_INCREMENT = 0.01f; // Redução no tempo por segundo
    private final float MINIMUM_POSSIBLE_SPAWN_TIME = 0.1f; // Tempo mínimo final
    ...
    public void render (float delta) {
        ...
        if (asteroidSpawnTimer <= 0) {
            // Atualiza os tempos mínimos e máximos dinamicamente
            currentMinSpawnTime = Math.max(MINIMUM_POSSIBLE_SPAWN_TIME, currentMinSpawnTime - DIFFICULTY_INCREMENT * delta);
            currentMaxSpawnTime = Math.max(MINIMUM_POSSIBLE_SPAWN_TIME, currentMaxSpawnTime - DIFFICULTY_INCREMENT * delta);

            // Recalcula o tempo para o próximo spawn
            asteroidSpawnTimer = random.nextFloat() * (currentMaxSpawnTime - currentMinSpawnTime) + currentMinSpawnTime;
        ...
        if (showControls) {
            if (SpaceGame.IS_MOBILE) {
                ...
                GlyphLayout instructionsLayout = new GlyphLayout(scoreFont, "Pressione as teclas direita/esqueda para se movimentar e atirar!", Color.WHITE, SpaceGame.WIDTH - 50, Align.center, true);
                scoreFont.draw(game.batch, instructionsLayout, (float) SpaceGame.WIDTH / 2 - instructionsLayout.width / 2, 150);
            }
        }
